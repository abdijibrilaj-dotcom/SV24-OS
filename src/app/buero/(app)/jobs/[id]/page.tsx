...
export default async function BueroJobDetailPage({
  params,
}: {
  ...
  return (
    <MobileScreen header={{ mode: "back", title: "Auftrag", backHref: "/buero/jobs" }} tabBar={undefined}>
      <div className="rounded-2xl border border-card-border-mobile bg-white p-[18px] shadow-[var(--shadow-card-mobile)]">
        ...
        <div className="mt-3.5 grid grid-cols-2 gap-3.5 text-[13px]">
          ...
          <div>
            <div className="text-[10px] uppercase tracking-[0.04em] text-text-tertiary">Dolmetscher</div>
            <div className="mt-0.5 font-medium">{job.interpreter?.name ?? "— offen —"}</div>
          </div>
          <div>
      ...
      {job.status === "CONFIRMED" && (
        <div className="mt-4">
          <Link
            href="/buero/kalender"
            className="block rounded-xl border border-field-border py-3 text-center text-sm font-bold"
          >
            Zum Kalender
          </Link>
        </div>
